function H_TranA = H_tran(A,transform)

%
% H_TranA = H_tran(A,transform) computes the conjugate transpose of an order-d(d>=3) tensor under linear transform
% 
% Input:
...
%
%
% Output: H_TranA     -   n2*n1*n3*¡­¡­*nd tensor
%
% References:
...
NwayA = size(A);
NdimA= ndims(A);
H_TranA=zeros([ size(A,2)  size(A,1)  NwayA(3:end) ]);

if nargin < 2
    % fft is the default transform
    transform.L = 'fft'; transform.rho = prod(NwayA(3:NdimA)); transform.inverseL = 'ifft';
end
    ...
    for j = 3 : NdimA
        for i = L(j-1)+1 : L(j)
            ...
            elseif I(j) > halfnj
                n_ = nc(I,j,NwayA);
                i_ = foldi(n_,j,L);
                H_TranA(:,:,i) = conj(H_TranA(:,:,i_));
            end
        end
    end
    
    for z = NdimA:-1:3
        H_TranA = (ifft(H_TranA,[],z));
    end
    %H_TranA=inverselineartransform(H_TranA);  
else
    
    LinearA = lineartransform(A,transform);
    for x = 1 : prod(NwayA(3:NdimA))
        H_TranA(:,:,x) = (LinearA (:,:,x))';
    end
    H_TranA=inverselineartransform(H_TranA,transform);
